#!/usr/bin/env bash
# =============================================================================
# run_fpn_lora_ema_ablation.sh — Experiment 6 (primary): deconv_v2 + FPN +
...
# based on raw weights, same as every other experiment. Instead, each seed
# now gets a THIRD test number: the EMA shadow stashed in seedN_final.ckpt is
...
#
# Checkpoint layout:
#   checkpoints/fpn-lora-ema-ablation/seed{N}/
...
BASE_CONFIG="configs/landmark/bpd_deconv_v2_fpn_lora_ema.yaml"
...
mkdir -p checkpoints/fpn-lora-ema-ablation
...
for SEED in "${SEEDS[@]}"; do
    ...
    RUN_NAME="fpn-lora-ema-ablation-seed${SEED}"
    RUN_DIR="checkpoints/fpn-lora-ema-ablation/seed${SEED}"
    TMP_CFG="/tmp/fpn_lora_ema_seed${SEED}.yaml"
...
base_cfg = sys.argv[2]
out_cfg  = sys.argv[3]
run_name = f'fpn-lora-ema-ablation-seed{seed}'
run_dir  = f'checkpoints/fpn-lora-ema-ablation/seed{seed}'
...
echo ""
echo "============================================================"
echo "  FPN + LoRA + EMA (deconv_v2, 64x64) — 5-seed ablation complete"
echo "============================================================"
echo ""
