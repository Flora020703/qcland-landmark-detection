#!/usr/bin/env bash
# =============================================================================
# run_fl_fpn_udp_rotate_scale_ablation.sh — Femur/FL, DINOv2, FPN+UDP +
# rotation + scale jitter, 5-seed. See run_apad_fpn_udp_rotate_scale_ablation.sh
# for full rationale. Different data from apad/tad (Femur, not Abdomen).
#
# Prerequisite: data must be on the server first —
#   /root/autodl-tmp/images/UCL/Femur/
#   /root/autodl-tmp/annotations/UCL/Femur_Train.csv
#   /root/autodl-tmp/annotations/UCL/Femur_Test.csv
#
# CANARY MODE:
#   CANARY_ONLY=1 bash ablation/scripts/run_fl_fpn_udp_rotate_scale_ablation.sh
#   bash ablation/scripts/run_fl_fpn_udp_rotate_scale_ablation.sh   # remaining 4 seeds
#
# Usage (on AutoDL server):
#   cd /root/eomt
#   nohup bash ablation/scripts/run_fl_fpn_udp_rotate_scale_ablation.sh > fl_fpn_udp_rotate_scale_ablation.log 2>&1 &
# =============================================================================

set -euo pipefail

BASE_CONFIG="configs/landmark/fl_dinov2_fpn_udp_rotate_scale.yaml"
...
RUN_GROUP="fl-dinov2-fpn-udp-rotate-scale"
RESULTS_TSV="fl_fpn_udp_rotate_scale_results.tsv"
DONE_MARKER="checkpoints/.fl_fpn_udp_rotate_scale_completed_seeds.txt"
BACKUP_ROOT="/root/autodl-tmp/saved_checkpoints/fl_fpn_udp_rotate_scale"

if [ ! -d /root/autodl-tmp/images/UCL/Femur ] || [ ! -f /root/autodl-tmp/annotations/UCL/Femur_Train.csv ]; then
    echo "[ERROR] Femur data not found on server — transfer images/UCL/Femur/ and annotations/UCL/Femur_*.csv first"
    exit 1
fi
...
checks = [
    ...
    ('images_dir',           data['images_dir'],             '/root/autodl-tmp/images/UCL/Femur'),
]
...
echo ""
echo "============================================================"
echo "  FL DINOv2 (FPN+UDP+rotate+scale) — 5-seed ablation summary"
echo "============================================================"
