#!/usr/bin/env bash
# =============================================================================
# run_fpn_udp_ema_ablation.sh — Experiment 8a: deconv_v2 + FPN + UDP + EMA,
# 5-seed ablation.
#
# First ablation to set loader_seed explicitly (= seed_everything per seed) —
# DataLoader shuffle order and multi-worker RNG seeding are now fully
# deterministic (datasets/landmark_dataset.py), fixing the "same seed,
# different result across script invocations" issue seen in the hm128 and
# FPN+EMA (Experiment 6) ablations.
#
# Baseline to beat: Experiment 7 (FPN+UDP, no EMA) — val-best 13.21%±1.92%,
# last 13.67%±1.08%, the best result so far.
#
...
# based on raw weights. Each seed gets a THIRD test number: the EMA shadow
# stashed in seedN_final.ckpt is materialized (apply_ema.py) and tested
# separately.
#
# Seeds: 42, 0, 123, 2024, 3407 (same set as the other ablations).
#
# Checkpoint layout:
#   checkpoints/fpn-udp-ema-ablation/seed{N}/
...
#   cd /root/eomt
#   bash ablation/scripts/run_fpn_udp_ema_ablation.sh
# =============================================================================

set -euo pipefail

BASE_CONFIG="configs/landmark/bpd_deconv_v2_fpn_udp_ema.yaml"
...
mkdir -p checkpoints/fpn-udp-ema-ablation
...
for SEED in "${SEEDS[@]}"; do
    ...
    RUN_NAME="fpn-udp-ema-ablation-seed${SEED}"
    RUN_DIR="checkpoints/fpn-udp-ema-ablation/seed${SEED}"
    TMP_CFG="/tmp/fpn_udp_ema_seed${SEED}.yaml"
...
base_cfg = sys.argv[2]
out_cfg  = sys.argv[3]
run_name = f'fpn-udp-ema-ablation-seed{seed}'
run_dir  = f'checkpoints/fpn-udp-ema-ablation/seed{seed}'
...
echo ""
echo "============================================================"
echo "  FPN + UDP + EMA (deconv_v2, 64x64) — 5-seed ablation complete"
echo "============================================================"
echo ""
...
echo "  FPN+LoRA (5-seed):            mean 17.22% ± 2.84% (val-best) / 18.30% ± 1.56% (last)  [worse]"
echo "  FPN+EMA, no loader_seed:      mean 17.86% ± 3.43% (val-best) / 17.01% ± 1.23% (last) / 15.82% ± 1.86% (EMA)"
echo "  FPN+UDP (5-seed):             mean 13.21% ± 1.92% (val-best) / 13.67% ± 1.08% (last)  [best so far]"
