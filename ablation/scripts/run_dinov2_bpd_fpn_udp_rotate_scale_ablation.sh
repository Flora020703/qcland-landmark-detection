#!/usr/bin/env bash
# =============================================================================
# run_dinov2_bpd_fpn_udp_rotate_scale_ablation.sh — BPD DINOv2 FPN+UDP +
# rotation + scale jitter, 5-seed. DINOv2 counterpart of
# run_dinov3_bpd_fpn_udp_rotate_scale_ablation.sh — see
# configs/landmark/bpd_dinov2_fpn_udp_rotate_scale.yaml for why this is
# tested on DINOv2 too rather than assuming the DINOv3 result transfers
# (this project has repeatedly shown it doesn't, for other interventions).
#
# References:
#   DINOv2 (no augment):     ensemble 9.75%, single-model last 14.68%±2.13%
#   DINOv3 (no augment):     ensemble 9.54%
#   DINOv3 + rotate:         ensemble (final) 10.34%
#   DINOv3 + rotate+scale:   ensemble (final) 10.40%  (did not beat 9.54%)
#   HRNet baseline:          8.00%
#
# Scale/rotate math + real-pipeline correctness already verified (backbone-
# agnostic, same augmentation code path):
...
#
# CANARY MODE:
#   CANARY_ONLY=1 bash ablation/scripts/run_dinov2_bpd_fpn_udp_rotate_scale_ablation.sh
#   bash ablation/scripts/run_dinov2_bpd_fpn_udp_rotate_scale_ablation.sh   # remaining 4 seeds
#
# Resumable (DONE_MARKER + checkpoint migration to data disk). If a seed's
# training crashes (e.g. disk full mid-write — has happened twice already
# this session), do NOT just clear it from DONE_MARKER and rerun in place;
# `rm -rf checkpoints/<run_group>/seed<N>` first for a fully clean retry —
# a crash-then-retry-in-the-same-directory produced one confirmed-bogus
# 57.86% NME result on the DINOv3 rotate+scale run (best==final exactly,
# a clean rewipe-and-retry gave a normal ~12% instead).
#
# Usage (on AutoDL server):
#   cd /root/eomt
#   nohup bash ablation/scripts/run_dinov2_bpd_fpn_udp_rotate_scale_ablation.sh > dinov2_bpd_fpn_udp_rotate_scale_ablation.log 2>&1 &
# =============================================================================

set -euo pipefail

BASE_CONFIG="configs/landmark/bpd_dinov2_fpn_udp_rotate_scale.yaml"
...
RUN_GROUP="bpd-dinov2-fpn-udp-rotate-scale"
RESULTS_TSV="dinov2_bpd_fpn_udp_rotate_scale_results.tsv"
DONE_MARKER="checkpoints/.dinov2_bpd_fpn_udp_rotate_scale_completed_seeds.txt"
BACKUP_ROOT="/root/autodl-tmp/saved_checkpoints/dinov2_bpd_fpn_udp_rotate_scale"
...
echo ""
echo "============================================================"
echo "  BPD DINOv2 + rotation + scale (FPN+UDP) — ablation summary"
echo "============================================================"
...
echo ""
echo "References: DINOv2 no-augment ensemble 9.75%, DINOv3 no-augment 9.54%,"
echo "DINOv3+rotate+scale ensemble 10.40% (did not beat 9.54%). HRNet: 8.00%"
echo ""
