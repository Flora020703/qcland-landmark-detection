#!/usr/bin/env bash
# =============================================================================
# run_dinov3_bpd_ablation.sh — DINOv2 vs DINOv3 backbone comparison, BPD leg.
#
# Config: bpd_dinov3_fpn_udp_ema.yaml (FPN+UDP+EMA, BPD's best config —
# identical to bpd_deconv_v2_fpn_udp_ema.yaml except backbone_name). This is
# the highest-priority DINOv3 run: BPD is the dataset still behind the HRNet
# baseline (12.09% vs 8.00%), so this result decides MICCAI vs MIDL framing.
#
# DINOv2 reference (5-seed, val-best / last):
#   deconv_v2 baseline:  15.18% ± 3.00% (last)
#   +FPN:                14.91% ± 2.94% (last)
#   +FPN+UDP:             14.68% ± 2.13% (last)   [loader_seed clean re-run]
#   +FPN+UDP+EMA:         12.09% ± 2.74% (last)   <- what DINOv3 needs to beat
#
# CANARY MODE: this script trains ONE seed only when CANARY_ONLY=1 is set.
# Run the canary first, sanity-check the NME against the DINOv2 numbers
# above (should be in the same ~8-17% ballpark, NOT off by an order of
# magnitude), THEN rerun without CANARY_ONLY to do the remaining 4 seeds —
# thanks to the DONE_MARKER resumability below, the canary seed is not
# retrained.
#
#   CANARY_ONLY=1 bash ablation/scripts/run_dinov3_bpd_ablation.sh   # ~1h, seed=42 only
#   bash ablation/scripts/run_dinov3_bpd_ablation.sh                 # remaining 4 seeds
#
# Before running either: python3 scripts/verify_dinov3_backbone.py must pass.
#
# Resumable (per project convention — a disk-full crash mid-ablation has
# bitten this project's scripts twice before, see run_300w_data_efficiency_
# 5seed_ablation.sh): a completed (seed) is recorded in DONE_MARKER and
# skipped on rerun. Checkpoints are moved off the system disk to
# /root/autodl-tmp/saved_checkpoints/ after each seed's NME is recorded.
#
# Usage (on AutoDL server):
#   cd /root/eomt
#   nohup bash ablation/scripts/run_dinov3_bpd_ablation.sh > dinov3_bpd_ablation.log 2>&1 &
# =============================================================================

set -euo pipefail

BASE_CONFIG="configs/landmark/bpd_dinov3_fpn_udp_ema.yaml"
...
RUN_GROUP="bpd-dinov3-fpn-udp-ema"
RESULTS_TSV="dinov3_bpd_results.tsv"
DONE_MARKER="checkpoints/.dinov3_bpd_completed_seeds.txt"
BACKUP_ROOT="/root/autodl-tmp/saved_checkpoints/dinov3_bpd"
...
" "$SEED" "$BASE_CONFIG" "$TMP_CFG" "$RUN_NAME" "$RUN_DIR"
    ...
    # --- 2. Verify critical hyperparams (only backbone_name + delta_weights
    #     differ from bpd_deconv_v2_fpn_udp_ema.yaml — everything else must
    #     match exactly, this is a controlled backbone-only comparison) ---
...
PYEOF
    ...
    # --- 6. Test val-best / last / EMA checkpoints ---
    for CKPT_TAG in best final ema; do
    ...
    # --- 7. Free system disk: move this seed's checkpoints off to the data disk ---
...
echo ""
echo "============================================================"
echo "  BPD DINOv3 (FPN+UDP+EMA) — ablation summary"
echo "============================================================"
...
echo ""
echo "DINOv2 reference (5-seed, last): FPN+UDP+EMA = 12.09% ± 2.74%"
echo "HRNet baseline: 8.00%"
echo ""
echo "If CANARY_ONLY=1 was used: rerun without it to train the remaining 4 seeds"
echo "(already-completed seeds are skipped via ${DONE_MARKER})."
echo ""
...
echo "      --ckpts ${BACKUP_ROOT}/seed*/seed*_best.ckpt"
echo ""
