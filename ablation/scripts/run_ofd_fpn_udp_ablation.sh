#!/usr/bin/env bash
# =============================================================================
# run_ofd_fpn_udp_ablation.sh — OFD counterpart of Exp 7 (FPN + UDP), 5-seed
# ablation.
#
# Same architecture/loss/backbone as BPD's best single config
# (configs/landmark/bpd_deconv_v2_fpn_udp.yaml): DeconvHead V2, FPN
# [4,8,12], UDP pixel_center_align, hybrid loss (lambda_coord=0.1, temp=10,
# alpha=5.0), 64x64 heatmap sigma=4.0, DINOv2-ViT-S/14 unfrozen. The only
# task-specific difference is data.init_args.task: ofd (selects
# ofd_1_x/y, ofd_2_x/y columns instead of bpd_1_x/y, bpd_2_x/y — see
# TASK_COLS in datasets/landmark_dataset.py). Same images, same
# Head_Train/Head_Test split, same seeds.
#
# Unlike the original BPD Exp 7 (which predates the loader_seed fix), this
# sets loader_seed = seed per run from the start (same pattern as Exp 8a) —
# see configs/landmark/ofd_deconv_v2_fpn_udp.yaml for why. No EMA: this
# reproduces "FPN+UDP only", the config whose raw best checkpoints ensemble
# best for BPD (9.75%) — see docs/experiment_summary_exp4-9_ensemble.md §4.
#
# Seeds: 42, 0, 123, 2024, 3407 (same set as every other ablation).
#
# Checkpoint layout:
#   checkpoints/ofd-fpn-udp-ablation/seed{N}/
#     seed{N}_best.ckpt   — saved by ModelCheckpoint
#     seed{N}_final.ckpt  — copy of last.ckpt
#     seed{N}_{best,final}_test_nme.txt
#
# After all 5 seeds finish, run the inference ensemble on the val-best
# checkpoints (not EMA, not last):
#   python3 ablation/ensemble_test.py \
#       --config configs/landmark/ofd_deconv_v2_fpn_udp.yaml \
#       --ckpts checkpoints/ofd-fpn-udp-ablation/seed*/seed*_best.ckpt
#
# Usage (on AutoDL server):
#   cd /root/eomt
#   bash ablation/scripts/run_ofd_fpn_udp_ablation.sh
# =============================================================================

set -euo pipefail

BASE_CONFIG="configs/landmark/ofd_deconv_v2_fpn_udp.yaml"
...
mkdir -p checkpoints/ofd-fpn-udp-ablation
...
for SEED in "${SEEDS[@]}"; do
    ...
    RUN_NAME="ofd-fpn-udp-ablation-seed${SEED}"
    RUN_DIR="checkpoints/ofd-fpn-udp-ablation/seed${SEED}"
    TMP_CFG="/tmp/ofd_fpn_udp_seed${SEED}.yaml"
...
base_cfg = sys.argv[2]
out_cfg  = sys.argv[3]
run_name = f'ofd-fpn-udp-ablation-seed{seed}'
run_dir  = f'checkpoints/ofd-fpn-udp-ablation/seed{seed}'
...
echo ""
echo "============================================================"
echo "  OFD: FPN + UDP coordinate fix (deconv_v2, 64x64) — 5-seed ablation complete"
echo "============================================================"
echo ""
...
echo ""
echo "For comparison (this is OFD's first architecture run, no prior OFD"
echo "einsum/deconv_v2/FPN ablations exist to compare against):"
echo "  HRNet OFD baseline (Di Vece et al.):  5.00%"
echo "  Flora's HRNet OFD reproduction:       4.41%"
echo "  BPD FPN+UDP for reference (5-seed):   mean 13.21% ± 1.92% (val-best) / 13.67% ± 1.08% (last)"
echo ""
echo "Next step: run the inference ensemble on the 5 val-best checkpoints:"
echo "  python3 ablation/ensemble_test.py --config ${BASE_CONFIG} \\"
echo "      --ckpts checkpoints/ofd-fpn-udp-ablation/seed*/seed*_best.ckpt"
echo ""
