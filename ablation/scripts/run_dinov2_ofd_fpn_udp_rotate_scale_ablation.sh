#!/usr/bin/env bash
# =============================================================================
# run_dinov2_ofd_fpn_udp_rotate_scale_ablation.sh — OFD DINOv2 FPN+UDP +
# rotation + scale jitter, 5-seed. See
# configs/landmark/ofd_dinov2_fpn_udp_rotate_scale.yaml for full rationale —
# OFD is now the paper's primary headline result (beats HRNet baseline
# already at 4.46%), so this checks whether augmentation pushes it further
# and whether OFD replicates BPD's "single-model up, ensemble diversity
# collapses" pattern or not.
#
# References:
#   DINOv2 OFD (no augment):  ensemble 4.46%, single-model last 4.86%±0.15%
#   DINOv3 OFD (no augment):  ensemble 4.54%, single-model last 4.62%±0.08%
#   HRNet baseline:            5.00%
#
# Rotation/scale math + real-pipeline correctness already verified
# (backbone/task-agnostic, same augmentation code as BPD):
...
#
# CANARY MODE:
#   CANARY_ONLY=1 bash ablation/scripts/run_dinov2_ofd_fpn_udp_rotate_scale_ablation.sh
#   bash ablation/scripts/run_dinov2_ofd_fpn_udp_rotate_scale_ablation.sh   # remaining 4 seeds
#
# Resumable (DONE_MARKER + checkpoint migration to data disk). If a seed's
# training crashes (e.g. disk full mid-write), do a full clean retry —
# `rm -rf checkpoints/<run_group>/seed<N>` first, not just clearing
# DONE_MARKER — a crash-then-retry-in-place has produced one confirmed
# bogus NME result already this project (best==final exactly was the tell).
#
# Usage (on AutoDL server):
#   cd /root/eomt
#   nohup bash ablation/scripts/run_dinov2_ofd_fpn_udp_rotate_scale_ablation.sh > dinov2_ofd_fpn_udp_rotate_scale_ablation.log 2>&1 &
# =============================================================================

set -euo pipefail

BASE_CONFIG="configs/landmark/ofd_dinov2_fpn_udp_rotate_scale.yaml"
...
RUN_GROUP="ofd-dinov2-fpn-udp-rotate-scale"
RESULTS_TSV="dinov2_ofd_fpn_udp_rotate_scale_results.tsv"
DONE_MARKER="checkpoints/.dinov2_ofd_fpn_udp_rotate_scale_completed_seeds.txt"
BACKUP_ROOT="/root/autodl-tmp/saved_checkpoints/dinov2_ofd_fpn_udp_rotate_scale"
...
echo ""
echo "============================================================"
echo "  OFD DINOv2 + rotation + scale (FPN+UDP) — ablation summary"
echo "============================================================"
...
echo ""
echo "References: DINOv2 no-augment ensemble 4.46%, DINOv3 no-augment 4.54%."
echo "HRNet baseline: 5.00% (both no-augment configs already beat this)."
echo ""
