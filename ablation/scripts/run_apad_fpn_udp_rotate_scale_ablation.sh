#!/usr/bin/env bash
# =============================================================================
# run_apad_fpn_udp_rotate_scale_ablation.sh — Abdomen/APAD, DINOv2, FPN+UDP
# + rotation + scale jitter (best config, HRNet-aligned recipe), 5-seed.
# See configs/landmark/apad_dinov2_fpn_udp_rotate_scale.yaml for full
# rationale (cross-anatomy generalizability, 2026-07-21).
#
# Prerequisite: data must be on the server first —
#   /root/autodl-tmp/images/UCL/Abdomen/
#   /root/autodl-tmp/annotations/UCL/Abdomen_Train.csv
#   /root/autodl-tmp/annotations/UCL/Abdomen_Test.csv
#
# CANARY MODE:
#   CANARY_ONLY=1 bash ablation/scripts/run_apad_fpn_udp_rotate_scale_ablation.sh
#   bash ablation/scripts/run_apad_fpn_udp_rotate_scale_ablation.sh   # remaining 4 seeds
#
...
#   cd /root/eomt
#   nohup bash ablation/scripts/run_apad_fpn_udp_rotate_scale_ablation.sh > apad_fpn_udp_rotate_scale_ablation.log 2>&1 &
# =============================================================================

set -euo pipefail

BASE_CONFIG="configs/landmark/apad_dinov2_fpn_udp_rotate_scale.yaml"
...
RUN_GROUP="apad-dinov2-fpn-udp-rotate-scale"
RESULTS_TSV="apad_fpn_udp_rotate_scale_results.tsv"
DONE_MARKER="checkpoints/.apad_fpn_udp_rotate_scale_completed_seeds.txt"
BACKUP_ROOT="/root/autodl-tmp/saved_checkpoints/apad_fpn_udp_rotate_scale"

if [ ! -d /root/autodl-tmp/images/UCL/Abdomen ] || [ ! -f /root/autodl-tmp/annotations/UCL/Abdomen_Train.csv ]; then
    echo "[ERROR] Abdomen data not found on server — transfer images/UCL/Abdomen/ and annotations/UCL/Abdomen_*.csv first"
    exit 1
fi
...
checks = [
    ...
    ('images_dir',           data['images_dir'],             '/root/autodl-tmp/images/UCL/Abdomen'),
]
...
echo ""
echo "============================================================"
echo "  APAD DINOv2 (FPN+UDP+rotate+scale) — 5-seed ablation summary"
echo "============================================================"
