#!/usr/bin/env bash
# =============================================================================
# run_fpn_udp_hm128_ablation.sh — Experiment 8b: deconv_v2 + FPN + UDP +
# 128x128 heatmap, 5-seed ablation.
#
# First ablation to set loader_seed explicitly (= seed_everything per seed) —
# see datasets/landmark_dataset.py and ablation/scripts/run_fpn_udp_ema_ablation.sh for why.
#
# Baseline to beat: Experiment 7 (FPN+UDP, 64x64) — val-best 13.21%±1.92%,
# last 13.67%±1.08%, the best result so far. hm128 alone (Experiment 3) gave
# no real gain at 64x64-baseline noise levels; re-testing now that UDP has
# removed a sample-dependent label-noise source that scales with heatmap
# resolution (see bpd_deconv_v2_fpn_udp_hm128.yaml for the reasoning).
#
# Seeds: 42, 0, 123, 2024, 3407 (same set as the other ablations).
#
# Checkpoint layout:
#   checkpoints/fpn-udp-hm128-ablation/seed{N}/
...
#   cd /root/eomt
#   bash ablation/scripts/run_fpn_udp_hm128_ablation.sh
# =============================================================================

set -euo pipefail

BASE_CONFIG="configs/landmark/bpd_deconv_v2_fpn_udp_hm128.yaml"
...
mkdir -p checkpoints/fpn-udp-hm128-ablation
...
for SEED in "${SEEDS[@]}"; do
    ...
    RUN_NAME="fpn-udp-hm128-ablation-seed${SEED}"
    RUN_DIR="checkpoints/fpn-udp-hm128-ablation/seed${SEED}"
    TMP_CFG="/tmp/fpn_udp_hm128_seed${SEED}.yaml"
...
base_cfg = sys.argv[2]
out_cfg  = sys.argv[3]
run_name = f'fpn-udp-hm128-ablation-seed{seed}'
run_dir  = f'checkpoints/fpn-udp-hm128-ablation/seed{seed}'
...
echo ""
echo "============================================================"
echo "  FPN + UDP + hm128 (deconv_v2) — 5-seed ablation complete"
echo "============================================================"
echo ""
...
echo "  hm128 alone (5-seed):         mean 15.16% ± 1.12% (val-best) / 15.48% ± 3.01% (last)"
echo "  FPN 64x64 (5-seed):           mean 14.49% ± 2.01% (val-best) / 14.91% ± 2.94% (last)"
echo "  FPN+UDP 64x64 (5-seed):       mean 13.21% ± 1.92% (val-best) / 13.67% ± 1.08% (last)  [best so far]"
