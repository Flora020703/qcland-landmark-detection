...
| `live_preflight.py` (added round 5, 4 of its own bugs fixed in round 6) | The enforcement mechanism for ENVIRONMENT.md's checklist. Round 6 found and fixed: the geometric round-trip check was wrongly using the AUGMENTED `train_dataloader` pipeline (now uses the deterministic `internal_val_dataloader`); it referenced `cfg.val_dataloader`, which is now `None` (now reads `cfg.internal_val_dataloader`); it never actually built/invoked `InternalFixedChannelNMEHook` from the registry (now does, via a minimal fake Runner exercising a real `mmengine.logging.MessageHub`); the forward/backward smoke test skipped `model.init_weights()` (now called). Also added a real BGR/RGB check (previously claimed, not implemented). Its `encode()`/`decode()` dict-key assumptions (`keypoint_x_labels`/`keypoint_y_labels`) are inferred from SimCCLabel's documented interface, not run against a live install -- if this specific step fails, that IS the preflight doing its job. |
| `internal_val_hook.py` (added round 5, fixed round 6) | Replaces MMPose's default `model.predict()`-based periodic validation. Round 6 fix (verified against the real MMEngine `Runner` source): reads `runner.cfg.internal_val_dataloader`, not `runner.cfg.val_dataloader` (which `make_config.py` now sets to `None`, required by Runner's own all-or-nothing `val_dataloader`/`val_cfg`/`val_evaluator` constraint). Device is no longer hardcoded `"cuda:0"` -- resolves to the model's own actual device at call time. Shares `low_level_decode.py`'s verified-safe decode path with `run_inference.py`. |
